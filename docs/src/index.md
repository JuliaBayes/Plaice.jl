...
VectorBijectors.to_vec
VectorBijectors.from_linked_vec
VectorBijectors.to_linked_vec
VectorBijectors.vec_length
VectorBijectors.linked_vec_length
VectorBijectors.optic_vec
VectorBijectors.linked_optic_vec
```
...
```@docs
VectorBijectors.has_constant_vec_bijector
```
...
```@docs
VectorBijectors.scalar_to_scalar_bijector
```
...
VectorBijectors.Log
VectorBijectors.Untruncate
```

## Testing

Because the scope of a vector bijector is very well-defined, there is a well-established testing framework to verify correctness of an implementation (`VectorBijectors.test_all()`), which you can use in the test suite.
