...
VB.to_linked_vec(d::UnconsMatrixDist) = VB.Vec(size(d))
VB.from_linked_vec(d::UnconsMatrixDist) = VB.Reshape(size(d))
VB.linked_vec_length(d::UnconsMatrixDist) = prod(size(d))
function VB.linked_optic_vec(d::UnconsMatrixDist)
    return map(c -> VarNames.Index(c.I, (;)), vec(CartesianIndices(size(d))))
end
...
VB.from_linked_vec(d::PDMatrixDistribution) = VB.InvPosDef(first(size(d)))
VB.to_linked_vec(d::PDMatrixDistribution) = VB.PosDef(first(size(d)))
function VB.linked_vec_length(d::PDMatrixDistribution)
    n = first(size(d))
    return div(n * (n + 1), 2)
end
VB.linked_optic_vec(d::PDMatrixDistribution) = fill(nothing, VB.linked_vec_length(d))

# LKJ correlation matrices.
#
# TODO(penelopeysm) VecCorrBijector has a few bugs. Look at the issue tracker.

VB.from_linked_vec(::D.LKJ) = VB.inverse(VB.VecCorrBijector())
VB.to_linked_vec(::D.LKJ) = VB.VecCorrBijector()
function VB.linked_vec_length(d::D.LKJ)
    n = first(size(d))
    return div(n * (n - 1), 2)
end
VB.linked_optic_vec(d::D.LKJ) = fill(nothing, VB.linked_vec_length(d))
