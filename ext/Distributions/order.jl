...
VB.to_linked_vec(d::D.OrderStatistic) = VB.to_linked_vec(d.dist)
VB.from_linked_vec(d::D.OrderStatistic) = VB.from_linked_vec(d.dist)
...
VB.linked_vec_length(d::D.JointOrderStatistics) = VB.vec_length(d)
# TODO: Technically, the first element can be @opticof(_[1]) so this is not technically
# correct.
VB.linked_optic_vec(d::D.JointOrderStatistics) = fill(nothing, VB.vec_length(d))
